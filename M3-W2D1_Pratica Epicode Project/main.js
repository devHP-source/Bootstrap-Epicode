// Strict template dataset containing your chosen favorite Zack Tabudlo records
const ALBUM_DATA = [
  {
    title: "Tunay",
    artist: "Zack Tabudlo",
...
// Garantisce che HTML è completamente caricato prima di eseguire qualsiasi manipolazione DOM
    ...
    builtMarkupString += `
      <div class="col-6 col-md-4 col-xl-3">
        <div class="card h-100 album-card text-light">
          ...
          <!-- Image Box Frame targeting flat square proportions -->
          <div class="card-img-container mb-3">
            <img src="${item.coverUrl}" class="card-img-top" alt="${item.title} Cover Graphic">
          </div>
          
          <!-- Text and Meta Elements Details wrapper -->
          <div class="card-body p-0 d-flex flex-column justify-content-between">
            <div class="mb-2">
              <h6 class="card-title text-truncate fw-bold mb-1 text-white" title="${item.title}">${item.title}</h6>
            ...
            <div class="pt-2 mt-1 border-top border-secondary-subtle">
              <div class="d-flex justify-content-between align-items-center" style="font-size: 0.75rem;">
                ...
                <span class="badge bg-dark border border-secondary text-white px-2 py-1" style="font-size: 0.65rem; font-weight: 500;">${item.type}</span>
              </div>
            </div>
          </div>
